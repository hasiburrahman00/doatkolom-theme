		...
		<div className="max-w-screen-xl lg:px-0 px-5 mx-auto py-12 md:py-20">
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
								...
								attributes.teachers_image?.map((item,index)=>(
									<div key={index} className="teachers w-10 h-10 rounded-full overflow-hidden">
										<a href={attributes.image_link}>
											<picture>
												<img className="object-cover w-full h-full lazyload" data-src={item} alt="teacher-image" />
											</picture>
										</a>
									</div>
								))
				...
				<div className="testimonial relative text-center lg:block hidden">
					<picture>
						<img className="object-cover lazyload" data-src={attributes.testimonial} alt="testimonial-bg" />
					</picture>
					<div className="testimonial-slides">
						{
							attributes.honorable_teacher?.map((item,index)=>(
								<div className="text-center" key={index}>
									<picture>
										<img className="object-cover w-full h-full rounded-full lazyload" data-src={item.image} alt="top-teachers" />
									</picture>
								</div>
							))
						}
