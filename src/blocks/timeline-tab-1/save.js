    ...
    return (
        <div className="py-16 md:py-28 bg-[#035769] px-5">
            <div className="max-w-screen-xl mx-auto">
                <div>
                    <h2 className="font-primary text-3xl md:text-5xl font-weight_secondary m-0 pb-10 md:p-0 text-[#C1F1FF] max-w-[662px] md:leading-14">{attributes.heading}</h2>
                </div>
                <div className="grid md:grid-cols-2 gap-10">
                    <div>
                        <div className="tab-content md:sticky md:top-[28vh] md:mt-20">
                                ...
                                <div className="md:pl-8 space-y-10">
                                    {
                                        attributes?.content?.map((item, index) => (
                                            <div key={index} className={`tab-item md:opacity-50 cursor-pointer group duration-200 transition ease-in text-white tab-item-index-${index}`} data-index={index}>
                                                <picture>
                                                    <img className="w-full block md:hidden mb-8 lazyload" data-src={item.img}/>
                                                </picture>
                                                <h2 className="m-0 p-0 text-2xl font-primary group-hover:underline font-weight_secondary mb-3">{item.title}</h2>
                        ...
                        <div className="doatkolom-image-tab">
                            {
                                attributes?.content?.map((item, index) => (
                                    <div key={index} className="image-tab-item min-h-[calc(100vh_-_100px)] flex items-center flex-col justify-center">
                                        <picture>
                                            <img className="w-full lazyload" data-src={item.img} data-index={index} id={`tab-image-index-${index}`}/>
                                        </picture>
                                    </div>
                                ))
                            }
