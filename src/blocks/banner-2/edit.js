export default function Edit({ attributes, setAttributes }) {
    ...
    wp.element.useEffect(()=>{
        if( !attributes.background ) {
            setAttributes({background: doatkolom_object.blocks + 'banner-2/img/banner-2-background.webp' })
        }
    },[])
                ...
                <div className="max-w-screen-xl mx-auto">
                    <div className="max-w-[620px]">
                        ...
                        { attributes.banner_btn.visible && 
                            <a className="transition duration-200 ease-linear no-underline font-weight_primary text-white py-3 px-8 bg-primary hover:bg-primary_dark font-secondary inline-block mt-7 rounded-md hover:text-white primary-shadow text-base" href="#">{attributes.banner_btn.text}</a>
                        }
                    </div>
                </div>
            </div>
