    ...
    <div class="max-w-screen-xl mx-auto px-5 flex justify-between items-center">
        <div>
            <?php if( !empty(INFO['official_email']) ): ?>
                <a class="text-white inline-block py-1.5 font-secondary no-underline hover:underline" href="mailto:<?php echo esc_html(INFO['official_email']) ?>"><?php echo esc_html(INFO['official_email']) ?></a> |
            <?php endif ?>
            <?php if( !empty(INFO['full_address']) ): ?>
                <a class="text-white inline-block py-1.5 font-secondary no-underline hover:underline" href="address"><?php echo esc_html(INFO['full_address']) ?></a>
            <?php endif ?>
        </div>
        <div class="space-x-6 flex items-center">
            <?php if( !empty(INFO['login_page']) ): ?>
                <a class="inline-flex py-1.5 items-center space-x-2 text-white font-secondary no-underline transition duration-200 hover:underline" target="_blank" href="<?php echo esc_url(INFO['login_page']) ?>">
                    <?php echo \DoatKolom\Icons::login() ?> <span><?php esc_html_e('Login/Register', 'doatkolom') ?></span>
                </a>
            <?php endif ?>
            <?php if( !empty(INFO['doatkolom_app_template']) ): ?>
